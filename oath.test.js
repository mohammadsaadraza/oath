...
describe("static methods", () => {
  it("resolve", () => {
    return Oath.resolve(DEFAULT_VALUE).then((v) =>
      expect(v).toEqual(DEFAULT_VALUE)
    );
  });

  it("reject", () => {
    return Oath.reject(DEFAULT_VALUE).catch((v) =>
      expect(v).toEqual(DEFAULT_VALUE)
    );
  });
  ...
  describe("all", () => {
    it("with success", () => {
      return Oath.all([promise({ value: 1 }), promise({ value: 2 })]).then(
        (v) => expect(v).toEqual([1, 2])
      );
    });

    it("with fail", () => {
      return Oath.all([promise(), promise({ fail: true })]).catch((v) =>
        expect(v).toEqual(DEFAULT_VALUE)
      );
    });
  });
  ...
  it("allSettled", () => {
    return Oath.allSettled([promise(), promise({ fail: true })]).then((v) =>
      expect(v).toEqual([
  ...
  describe("race", () => {
    it("with success", () => {
      return Oath.race([promise({ value: 1 }), promise({ value: 2 })]).then(
  ...
  describe("any", () => {
    it("with success", () => {
      return Oath.any([promise({ value: 1 }), promise({ value: 2 })]).then(
        (v) => expect(v).toEqual(1)
      );
    });
